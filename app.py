...
def init_db():
    ...
    ''')
    # Add website column if missing (migration for existing databases)
    try:
        db.execute("ALTER TABLE centers ADD COLUMN website TEXT DEFAULT ''")
    except:
        pass
...
def index():
    ...
    for c in centers:
        ...
        centers_data.append({
            ...
            'telegram': c['telegram'], 'website': c['website'],
...
def save_center_info():
    ...
    db.execute('''UPDATE centers SET name=?, city=?, address=?, phone=?, hours=?,
                  description=?, instagram=?, telegram=?, website=? WHERE id=?''',
